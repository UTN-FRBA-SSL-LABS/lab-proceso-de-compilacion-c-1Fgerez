...
> **R:** Proque la directiva #include hace que el preprocesador copie y pegue el contenido de los headers dentro del codigo.
...
> **R:** Porque el preprocesador actua como un limpiador y elimina todos los comentarios, ya que al compilador no le importa.
...
> **R:**1er comando: NADA. 2do comando: printf("[DEBUG] %s\n", ("Iniciando main"));
...
> **R:**Son marcadores de linea que el preprocesador envia para saber el origen de cad bloque de codigo. Sirve para que el compilador pueda reportar errores indicando el archivo y la linea original. Proviene de /usr/include/stdio.h
...
> **R:**Se encarga de preparar la funcion antes de ejecutar las operaciones principales. Sirve para establecer el marco de la funcion, reservar espacio en memoria y guardar los parametros o el estado inicial de los registros.
...
> **R:**Lo que pasa es que el programa.o llama a la funcion pero no la tiene, matematica.o es el archivo que tiene el codigo que la define
...
> **R:**Quedan porque la funcion printf pertenece a la biblioteca libc, por lo que el enlazador no copia su codigo en el ejecutable, deja una referencia para que el sistema operativo las resuelva en tiempo de ejecucion
...
> **R:**La macro funcion se reemplaza con texto en la etapa de preprocesamiento, por eso no se verifica el tipo. En cambio, la funcion real es analizada por el compilador donde si se verifica el tipo. Luego se resuelve en la etapa de enlazado
...
> **R:**El simbolo T indica que es una funcion, es codigo ejecutable. Y vive en la seccion .text del archivo objeto. En cambio, el simbolo D es una variable global, la cual tiene un valor inicial, vive en la seccion .data
...
> **R:**ldd programa.exe
        ntdll.dll => /c/WINDOWS/SYSTEM32/ntdll.dll (0x7fff34680000)
        KERNEL32.DLL => /c/WINDOWS/System32/KERNEL32.DLL (0x7fff33210000)
        KERNELBASE.dll => /c/WINDOWS/System32/KERNELBASE.dll (0x7fff32010000)
        msvcrt.dll => /c/WINDOWS/System32/msvcrt.dll (0x7fff32410000)

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:**Porque gcc actua como intermediario que llama al enlazador pasandole los parametros incluyendo automaticamente la biblioteca estandar de C
